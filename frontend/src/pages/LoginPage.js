...
export default function LoginPage() {
  ...
  return (
    <div data-testid="login-page" className="min-h-screen bg-[#F9F6F0] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="font-['Outfit'] text-3xl font-semibold text-[#1A2E24]">Bem-vindo de volta</h1>
          <p className="text-sm text-[#4A6B5A] mt-2">Entre na sua conta para acessar seu painel</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white/60 backdrop-blur-sm border border-[#E0DDD5] rounded-3xl p-8 shadow-[0_8px_32px_rgba(44,76,59,0.04)] space-y-5">
          {error && (
            <div data-testid="login-error" className="bg-red-50 border border-red-200 text-red-700 text-sm rounded-xl px-4 py-3">
              {error}
            </div>
          )}
